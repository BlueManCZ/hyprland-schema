...
from hyprland_schema._schema import Schema, source_url
...
def _fetch_or_read(input_path: Path | None, version: str) -> str:
    ...
    try:
        content, url = fetch_header(version)
...
def prune_redundant_migrations(output_dir: Path, snapshots: set[str]) -> list[Path]:
    """Delete migration files whose ``to_version`` already has a snapshot.

    Resolution walks back from the target to the nearest snapshot at or newer
    than it, then applies migrations forward toward the target. Any migration
    whose ``to_version`` is itself a snapshot is therefore never traversed —
    it's pure disk weight and adds confusion about what's authoritative.
    """
    mig_dir = _migrations_dir(output_dir)
    pruned: list[Path] = []
    for f in mig_dir.iterdir():
        if f.suffix != ".json" or f.name.endswith("_snapshot.json"):
            continue
        if "_to_" not in f.stem:
            continue
        _, _, to_ver = f.stem.partition("_to_")
        if to_ver in snapshots:
            f.unlink()
            pruned.append(f)
    return pruned
...
def _cmd_bump(version: str, input_path: Path | None, output_dir: Path) -> None:
    ...
    # A mid-chain snapshot can supersede a migration we just wrote — for example
    # when bumping leaves the chain at [new, old, even-older] and the snapshot
    # lands at `old`, the new→old migration is never traversed at runtime.
    _, _, snapshot_set = update_registry(output_dir)
    pruned = prune_redundant_migrations(output_dir, snapshot_set)
    for p in pruned:
        print(f"Pruned redundant migration {p.name}", file=sys.stderr)
    if pruned:
